...
GtkWidget *getTelaGame() {
    ...
    gerarTabela();   // Gerando layout da partida

    printTabela();   // Mostra o layout no console
...
void flagCelula(GtkGestureClick* gesture_click, gint n, gdouble x, gdouble y, gpointer data){
    ...
    //Caso !revelado e em progresso
    if (but_data->status != 1 && gameStatus == 0) {
        atualizarBandLabel();
        int bombasSalvas = 0;      // Bombas com flags
        ...
        printf("Bombas salvas: %d\n", bombasSalvas);
        if (bombasSalvas == bombas) {
            endDialog("win");
            atualizarVitorias(1);
        }
    }
...
void clickCelula(GtkButton* gtk_button, gpointer data){
    ...
    if (but_data->status == 0) {
        ...
        // tempString para label do botão
        ...
        for (int i=(but_data->x)-2; i<=(but_data->x)+2; i++) {
            for (int j=(but_data->y)-2; j<=(but_data->y)+2; j++) {
                ...
                if (tabela[i][j].status == 0) {
                    // Caso botão clicado é bom e o jogo está em progresso
...
GtkWidget *getBotaoTabela(Celula* celula) {
    ...
    g_signal_connect(mainButton, "clicked", G_CALLBACK(clickCelula), button_data);   // Botão esquerdo do mouse
    g_signal_connect(gesture[l][c], "pressed", G_CALLBACK(flagCelula), button_data);  // Botão direito do mouse
    ...
    // Caso vazio e ponto seguro !revelado
...
void atualizarVitorias(int n /* zerar = 0 , adicionar = 1 */) {
    FILE *arq;
